...
public final class ObserverRemoteCursorRelayManager {
    ...
    public static void accept(ServerPlayer target, ObserverRemoteCursorPayloads.State state) {
        ...
        boolean owned = ObserverOwnedScreenRelayManager.matchesOpen(
                targetId, state.familyId(), state.variant(), state.screenProtocol());
        List<UUID> familyObservers;
        if (owned) {
            if (!ObserverNativeSessionManager.ownedProviderAdvertises(
                    target, state.familyId(), state.screenProtocol())) return;
            familyObservers = ObserverNativeSessionManager.observerIdsForTarget(
                    ...
                    .filter(observerId -> {
                        ...
                        return observer != null && ObserverNativeSessionManager.ownedProviderAdvertises(
                                observer, state.familyId(), state.screenProtocol());
                    })
                    .toList();
        } else {
            long vanillaCapability = ObserverScreenCapabilities.vanillaCapability(state.familyId());
            if (vanillaCapability == 0L) return;
            familyObservers = ObserverNativeSessionManager.observerIdsForTarget(targetId, vanillaCapability);
        }

        if (state.sequence() <= LAST_SEQUENCE.getOrDefault(targetId, Map.of())
                .getOrDefault(state.familyId(), -1L)) return;
        ...
        var relay = new ObserverRemoteCursorPayloads.Relay(targetId, state.protocolVersion(), state.sequence(),
                state.familyId(), state.variant(), state.screenProtocol(), state.x(), state.y(),
                state.contentWidth(), state.contentHeight(), state.carried().copy());
    ...
    public static void clearTarget(UUID targetId) {
        LAST_SEQUENCE.remove(targetId);
        LAST_ACCEPTED_NANOS.remove(targetId);
    }
