...
test -f "$lockstep_root/TotemAutomata/build/libs/totem-automata-0.1.16.jar"

"$wrapper" -p "$lockstep_root/TotemNexus" \
  -PtotemCoreJar="$core_jar" jar --no-daemon --stacktrace
test -f "$lockstep_root/TotemNexus/build/libs/totem-nexus-0.3.5.jar"

"$wrapper" -p "$lockstep_root/TotemVillagers" \
  -PtotemCoreJar="$core_jar" -PtotemRemnantJar="$remnant_jar" \
  jar --no-daemon --stacktrace
test -f "$lockstep_root/TotemVillagers/build/libs/totem-villagers-0.1.32.jar"

"$wrapper" -p "$lockstep_root/TotemLocksmith" \
  -PtotemCoreJar="$core_jar" jar --no-daemon --stacktrace
test -f "$lockstep_root/TotemLocksmith/build/libs/totem-locksmith-0.1.5.jar"
