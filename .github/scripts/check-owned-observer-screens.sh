...
if grep -En -- "${legacy_module_clients}\.register\(\)" "$root/src/main/java/dev/totem/vanillatweaks/TotemVanillaTweaksClient.java"; then
  echo 'A duplicated module UI adapter is still registered.' >&2
  exit 1
fi
if grep -ERn -- 'class[[:space:]]+[A-Za-z0-9_]*(Mirror|Lookalike)Screen[[:space:]]+extends' "$clients"; then
...
for contract in ObserverReadOnlyScreen ObserverOwnedScreenCoordinator ObserverScreenProviders; do
  grep -ERq -- "$contract" "$root/src/main" || { echo "Missing owner-screen contract: $contract" >&2; exit 1; }
done
...
for module in "${!owner_screens[@]}"; do
  ...
  grep -ERq -- "class[[:space:]]+${owner_screens[$module]}" \
