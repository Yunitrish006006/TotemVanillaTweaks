## TotemVanillaTweaks 0.1.19 Beta

### Publication correction

- Carries the complete Observer update forward under a new version because an earlier 0.1.18 artifact already exists on Modrinth and must not be overwritten.
- Declares the verified `totem-core-0.7.11.jar` as an exact required external file dependency instead of associating this release with an unrelated same-name Modrinth project.
- Builds the release JAR from a clean output directory and reports explicit, non-secret validation errors for release metadata and existing-version conflicts.
